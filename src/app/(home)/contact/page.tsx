...
export default function ContactPage() {
  ...
  const inputBase = "w-full rounded-xl border bg-white px-4 py-3.5 text-sm text-gray-800 placeholder-gray-400 outline-none transition-all duration-200";
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-white">
      ...
      <section className="relative overflow-hidden px-6 pb-16 pt-24">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="text-base text-gray-500 md:text-lg">
        ...
        >
          {features.map(({ icon: Icon, title, desc, color, bg }) => (
            <motion.div key={title} variants={fadeUp}
              className={`rounded-2xl border p-5 ${bg} transition-all duration-200 hover:-translate-y-1 hover:shadow-md`}
            >
              ...
              <h3 className="mb-1.5 font-semibold text-gray-800">{title}</h3>
          ...
          <AnimatePresence mode="wait">
            {status === "success" ? (
              <motion.div key="success"
                ...
                className="flex flex-col items-center gap-4 rounded-3xl border border-emerald-100 bg-gradient-to-b from-emerald-50 to-white p-12 text-center shadow-lg"
              >
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                  <CheckCircle size={32} />
                </div>
                <h2 className="text-xl font-bold text-gray-900">Message envoyé !</h2>
                <p className="text-gray-500">Merci pour votre intérêt. Nous vous contacterons sous 24h.</p>
              ...
              <motion.form key="form" onSubmit={handleSubmit}
                className="rounded-3xl border border-gray-100 bg-white p-8 shadow-xl shadow-gray-100"
              >
                <h2 className="mb-6 text-xl font-bold text-gray-900">Envoyez-nous un message</h2>
                  ...
                  {[
                    ...
                    <div key={name}>
                      ...
                      <input
                        ...
                        required
                        className={`${inputBase} ${focused === name ? "border-blue-400 ring-2 ring-blue-100" : "border-gray-200 hover:border-gray-300"}`}
                      />
                    </div>
                  ))}
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold text-gray-500 uppercase tracking-wider">Votre message</label>
                    <textarea
                      name="message" placeholder="Décrivez votre flotte ou votre besoin..."
                      value={form.message} onChange={handleChange}
                      onFocus={() => setFocused("message")} onBlur={() => setFocused(null)}
                      rows={4}
                      className={`${inputBase} resize-none ${focused === "message" ? "border-blue-400 ring-2 ring-blue-100" : "border-gray-200 hover:border-gray-300"}`}
                    />
                  </div>
